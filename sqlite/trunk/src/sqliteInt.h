...
struct ExprList {
  ...
  ExprList_item *a;      /* One entry for each expression */
};
...
struct SrcList {
  ...
  SrcList_item a[1];  /* One entry for each identifier on the list */
};
...
union bar {               /* Information that depends on plan.wsFlags */
  foo in;                 /* Used when plan.wsFlags&WHERE_IN_ABLE */
};
...
struct Parse {
  ...
  yColCache aColCache[SQLITE_N_COLCACHE];  /* One for each column cache entry */
