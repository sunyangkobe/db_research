...
struct WalIterator {
  ...
  WalSegment aSegment[1];                  /* One for every 32KB page in the wal-index */
};
...
int sqlite3WalFrames(
  ...
  */
  ...
  for(p=pList; p; p=p->pDirty){
    ...
    pData = p->pData;
#endif
    walEncodeFrame(pWal, p->pgno, nDbsize, (u8 *) pData, aFrame);
  ...
  if( sync_flags ){
    ...
    while( iOffset<iSegment ){
      ...
      pData = pLast->pData;
#endif
      walEncodeFrame(pWal, pLast->pgno, nTruncate, (u8 *) pData, aFrame);
      /* testcase( IS_BIG_INT(iOffset) ); // requires a 4GiB WAL */
      rc = sqlite3OsWrite(pWal->pWalFd, aFrame, sizeof(aFrame), iOffset);
      if( rc!=SQLITE_OK ){
        return rc;
