...
Expr *sqlite3ExprAlloc(
  ...
  pNew = (Expr *)sqlite3DbMallocZero(db, sizeof(Expr)+nExtra);
  if( pNew ){
    pNew->op = (u8)op;
    pNew->iAgg = -1;
    if( pToken ){
...
static Expr *exprDup(sqlite3 *db, Expr *p, int flags, u8 **pzBuffer){
  ...
  if( p ){
    ...
    if( pzBuffer ){
      ...
      zAlloc = (u8 *)sqlite3DbMallocRaw(db, dupedExprSize(p, flags));
    }
    pNew = (Expr *)zAlloc;
...
ExprList *sqlite3ExprListDup(sqlite3 *db, ExprList *p, int flags){
  ...
  pNew->a = pItem = (ExprList_item *)sqlite3DbMallocRaw(db,  p->nExpr*sizeof(p->a[0]) );
  if( pItem==0 ){
...
IdList *sqlite3IdListDup(sqlite3 *db, IdList *p){
  ...
  pNew->a = (IdList_item *)sqlite3DbMallocRaw(db, p->nId*sizeof(p->a[0]) );
  if( pNew->a==0 ){
    sqlite3DbFree(db, pNew);
    return 0;
  }
...
ExprList *sqlite3ExprListAppend(
  ...
  if( pList->nAlloc<=pList->nExpr ){
    ...
    a = (ExprList_item *)sqlite3DbRealloc(db, pList->a, n*sizeof(pList->a[0]));
    if( a==0 ){
      goto no_mem;
    }
    pList->a = a;
...
int sqlite3ExprCodeTarget(Parse *pParse, Expr *pExpr, int target){
  ...
  switch( op ){
    case TK_AGG_COLUMN: {
      ...
      struct AggInfo_col *pCol = (struct AggInfo_col *)&pAggInfo->aCol[pExpr->iAgg];
...
static int addAggInfoColumn(sqlite3 *db, AggInfo *pInfo){
  int i;
  pInfo->aCol = (AggInfo_col *)sqlite3ArrayAllocate(
       db,
       pInfo->aCol,
       sizeof(pInfo->aCol[0]),
       3,
...
static int addAggInfoFunc(sqlite3 *db, AggInfo *pInfo){
  int i;
  pInfo->aFunc = (AggInfo_func *)sqlite3ArrayAllocate(
       db, 
       pInfo->aFunc,
