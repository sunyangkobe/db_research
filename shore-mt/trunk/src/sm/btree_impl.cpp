...
/*<std-header orig-src='shore'>
 ...
 $Id: btree_impl.cpp,v 1.51 2010/12/17 19:36:26 nhall Exp $
...
// BTREE_CHECK_LATCHES: turn it on in btree_impl.h 
...
)
{
    ...
    W_DO( ret_page.fix(pid, LATCH_EX, ret_page.t_virgin, stf) );
...
again:
    {
        ...
        w_assert2( parent.is_fixed());
        w_assert2( parent.is_node() || (parent.is_leaf() &&
                    leaf.pid() == root  ));
        ...
        if(leaf.is_smo() || leaf.is_delete()) {
             ...
             */
            w_error_t::err_num_t rce = tree_root.get_for_smo(true, LATCH_SH, 
                        leaf, LATCH_EX, false, &parent, LATCH_NL);
            ...
            if(rc.is_error()) {
                ...
                if (rc.err_num() == eRECWONTFIT) {
                    ...
                    w_error_t::err_num_t rce = 
                        tree_root.get_for_smo(true, LATCH_EX,
                            leaf, LATCH_EX, false, 0, LATCH_NL);
...
again:
    ...
    if(leaf.is_smo()) {
        w_error_t::err_num_t rce;
        rce = tree_root.get_for_smo(true, LATCH_SH,
                    leaf, LATCH_EX, false, &parent, LATCH_NL);
    ...
    {   /* 
        ...
        if (leaf.nrecs() == 0)  {
             ...
             */
            w_error_t::err_num_t rce;
            rce = tree_root.get_for_smo(true, LATCH_EX,
                    leaf, LATCH_EX,
                    true, &sib, LATCH_EX);
     ...
     again:
        ...
        case m_not_found_end_of_non_empty_page: {
            ...
            if(whatcase == m_satisfying_key_found_same_page) {
                ...
                // w_error_t::err_num_t rce;
                // unconditional
                (void) /*rce =*/ tree_root.get_for_smo(false, LATCH_SH,
                        leaf, LATCH_SH, false, &p2, LATCH_NL);
                ...
                //filed in GNATS 137
                // W_IGNORE(rce); // rce is no longer an rc_t
...
btree_impl::_grow_tree(btree_p& rp)
{
     ...
     *  Sanity check
     */
    rp.upgrade_latch(LATCH_EX);
    w_assert9(rp.latch_mode() == LATCH_EX);
    w_assert1(rp.next());
    w_assert9(rp.is_smo());
    ...
    }
    {
        ...
        while ( top >= 0 )  {
            ...
            if( top == 0 ) {
                ...
                if(isdelete) {
                    W_DO(_shrink_tree(p[top]));
                    // we're done?
                    break;
                } else {
                    p[top].fix(root_pid, LATCH_EX);
                    btree_p &rootp = p[top];
                    W_DO(_grow_tree(rootp));
                    w_assert9(rootp.nrecs() == 0);
                    w_assert9(rootp.pid0());
                    w_assert9(rootp.next()==0);
                    child_pid.page = rootp.pid0();
                    // have to do one more 
...
)
{
    ...
    X_DO(_propagate(root_pid, key, el, leaf_pid, level, false /*not delete*/), anchor);
    ...
    if (xd)  {
        ...
        xd->compensate(anchor,false/*not undoable*/ X_LOG_COMMENT_USE("btree.prop.1"));
    }
...
pagain:
...

    {
        ...
        DBGTHRD(<<"SH fix parent to check if it's a leaf");
